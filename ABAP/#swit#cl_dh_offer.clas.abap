class /SWIT/CL_DH_OFFER definition
  public
...
CLASS /SWIT/CL_DH_OFFER IMPLEMENTATION.


  method /SWIT/IF_DH_FACTORY~NEW.

    ro_instance = new /swit/cl_dh_offer( ).
  ...
  METHOD is_in_data.
    ...
    ASSIGN COMPONENT 'SHORTTITLE'   OF STRUCTURE is_data TO FIELD-SYMBOL(<shorttitle>).
    ...
    IF ( lv_my_posts_flag EQ abap_true AND lv_userid IS NOT INITIAL AND lv_userid EQ <userid> ) OR lv_my_posts_flag EQ abap_false.
      ...
      IF <uname> CS lv_basic_search OR <street> CS  lv_basic_search OR <postcode> CS lv_basic_search OR <city> CS lv_basic_search OR <title> CS lv_basic_search OR <shorttitle> CS lv_basic_search OR <descr> CS lv_basic_search OR lv_basic_search IS INITIAL.
