class /SWIT/CL_DH_ACCOM definition
  public
...
CLASS /SWIT/CL_DH_ACCOM IMPLEMENTATION.


  method /SWIT/IF_DH_FACTORY~NEW.

    ro_instance = new /swit/cl_dh_accom( ).
  ...
  METHOD is_in_data.
    ...
    ASSIGN COMPONENT 'COST'         OF STRUCTURE is_data TO FIELD-SYMBOL(<cost>).
    ASSIGN COMPONENT 'AREA'         OF STRUCTURE is_data TO FIELD-SYMBOL(<area>).
    ASSIGN COMPONENT 'ROOMS'        OF STRUCTURE is_data TO FIELD-SYMBOL(<rooms>).
    ...
    IF ( lv_my_posts_flag EQ abap_true AND lv_userid IS NOT INITIAL AND lv_userid EQ <userid> ) OR lv_my_posts_flag EQ abap_false.
      ...
      IF me->has_access( iv_me = lv_userid iv_other_userid = <userid> ) EQ abap_true.
        ...
        IF <uname> CS lv_basic_search OR <street> CS  lv_basic_search OR <postcode> CS lv_basic_search OR <city> CS lv_basic_search OR <title> CS lv_basic_search OR <cost> CS lv_basic_search OR <area> CS lv_basic_search OR <rooms> CS lv_basic_search OR
           <descr> CS lv_basic_search OR lv_basic_search IS INITIAL.
